...
namespace :imodec do
	task :seo=>:environment do
		...
		Ecstore::Brand.unscoped.all.each do |brand|
			if brand.meta_seo
				brand.meta_seo.update_attributes(:title=>"#{brand.brand_name} | scnc-sh | 呵护儿童健康",
									    :keywords =>brand.seo&&brand.seo.keywords,
									    :description =>brand.seo&&brand.seo.description)

				puts "=============updating brand `#{brand.brand_name}` ok============="
			else
				brand.create_meta_seo(:title=>"#{brand.brand_name} | scnc-sh | 呵护儿童健康",
									    :keywords =>brand.seo&&brand.seo.keywords,
									    :description =>brand.seo&&brand.seo.description)
		...
		Ecstore::Category.all.each do |cat|
			...
			if cat.meta_seo
				cat.meta_seo.update_attributes(:title=>"#{cat.cat_name} | scnc-sh | 呵护儿童健康",
				...
				cat.create_meta_seo(:title=>"#{cat.cat_name} | scnc-sh | 呵护儿童健康",
									    :keywords =>cat.seo&&cat.seo.keywords,
									    :description =>cat.seo&&cat.seo.description)
