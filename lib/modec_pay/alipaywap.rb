...
module ModecPay
    ...
    @@seller_account_name='maowei@iotps.com'
      ...
      #self.action='http://mall.scnc-sh.com/payments/debug'
    ...
    #  self.sorter = ['service','req_data','partner','req_id','sec_id', 'format','v']
      ...
      @merchant_url = 'http://mall.scnc-sh.com/m' #操作中断返回地址,可空
        ...
        unsign = unsign_hash.collect do |key,val| 	"#{key}=#{val}" end.join("&") + @@private_key #self.private_key
			...
			def verify_notify(params,options)
				if verify_sign(params)
					...
					ModecPay.logger.info "[alipaywap][#{Time.zone.now}] payment=#{params['out_trade_no']} verify notify successfully."
          ...
          case params['result']
			...
			def verify_return(params,options)
				if verify_sign(params)
					ModecPay.logger.info "[alipaywap][#{Time.zone.now}] verify return successfully."
					case params['result']
						when 'success'
