  ...
  if params[:ship_status] == '1'
    ...
    if Ecstore::WeihuoShare.where(:order_id => params[:order_id]).first.payment_method == 'scan_qrcode'
      ...
      if Ecstore::WeihuoShare.where(:order_id => order_id).present? && Ecstore::WeihuoShare.where(:order_id => order_id).first.status == 0
        ...
        data[:wishing] = '加油加油加油！！！'
        data[:act_name] = shop_id == 49 ? '上儿官网随机红包' : '医生'
...
def show
  ...
  # if !current_account.present? || !current_account.login_name.presents? || current_account.login_name.split('_shop_')[1].split('_')[0] != shop_id
  #   return redirect_to "/shop_login?id=78&shop_id=#{shop_id}"
  # end

  openid, shopid= current_account.login_name.split('_shop_') if current_account
  ...
  def share
    @url = "http://mall.scnc-sh.com/weihuo/shops/#{params[:shop_id]}"
  ...
  def pay_with_goods bn
    ...
    supplier = Ecstore::Supplier.where(:name => '上儿').first
    weixin_appid = supplier.weixin_appid
    weixin_appsecret = supplier.weixin_appsecret
    mch_id = supplier.mch_id
...
def create_sign hash
  key = Ecstore::Supplier.where(:name => '上儿').first.partner_key
