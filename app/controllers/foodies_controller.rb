...
class FoodiesController < ApplicationController
  ...
  def foodie_pay
    ...
    notify_url = 'http://www.scnc-sh.com/foodie_notify_url'
    ...
    if res_data_hash["xml"]["return_code"] == 'SUCCESS'
      @url = "http://www.scnc-sh.com/foodiegroup/#{params[:type_name]}/#{params[:parent_id]}?from=foodiepay&total=#{total_fee}"
  ...
  def foodie_notify_url
    url = "http://www.scnc-sh.com/foodiegroup/wechat_notify_url"
  ...
  def foodie_group_share
    ...
    post_params = {
      ...
      :timestamp => @timestamp,
      :url => request.url.gsub("trade", "www.scnc-sh.com")
    }
    @sign = create_sign_for_js post_params
    @a = [post_params, request.url.gsub("trade", "www.scnc-sh.com")]
    render :layout => false
  end
