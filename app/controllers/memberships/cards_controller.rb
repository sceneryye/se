...
class Memberships::CardsController < ApplicationController
	...
	def confirm_user
		...
		if @card.save(:validate=>false)
			...
			# 	text = "您已成功购买摩登客VIP卡,感谢您对我们的支持，如需帮助可致电客服021-37561067[I-Modec摩登客]"
			...
			@error_msg = "提交订单失败，请联系客服:021-37561067"
	...
	def validate_activation
		...
		if @card.can_use?
			...
			text = "您的VIP卡验证码是：#{sms_code}，如此条验证码非您本人申请，请立即致电客服021-37561067核实[I-Modec摩登客]"
			@sms_log ||= Logger.new('log/sms.log')
			begin
	...
	def activate
		...
		if @card.can_use? && !@card.used?
			...
			begin
				@sms_log ||= Logger.new('log/sms.log')
				text = "您购买的摩登客VIP卡#{@card.no}已被#{mask @card.member_card.user_tel}激活,如有疑问请致电021-37561067[I-Modec摩登客]"
				if Sms.send(@card.member_card.buyer_tel,text)
					tel = @card.member_card.buyer_tel
					@sms_log.info("[#{@user.login_name}][#{Time.zone.now}][#{tel}]#{text}")
				end

				text = "您的摩登客VIP卡#{@card.no}已激活,如有疑问请致电客服021-37561067[I-Modec摩登客]"
