module SessionsHelper
  ...
  def sing_shop_in(accounts,remember_me = nil)
    ...
    if Rails.env == "production"
      cookies[:loginName] = {:value=>accounts.visitor_name,:domain=>'.scnc-sh.com'}
      cookies[:UNAME] = {:value=>accounts.visitor_name,:domain=>'.scnc-sh.com'}
      # cookies[:MLV] = {:value=>account.user.member_lv_id,:expires=>expires,:domain=>'.scnc-sh.com'}
      #cookies[:CUR] = {:value=>account.user.cur,:expires=>expires,:domain=>'.scnc-sh.com'}
      # cookies[:LANG] = {:value=>account.user.lang,:expires=>expires,:domain=>'.scnc-sh.com'}
    else
  ...
  ###
	...
	def sign_in(account,remember_me = nil)
	   current_account = account
         ...
         if Rails.env == "production"
	         cookies[:MEMBER] = {:value=>account.gen_secret_string_for_cookie,:expires=>expires,:domain=>'.scnc-sh.com'}
	         cookies[:loginName] = {:value=>account.login_name,:expires=>expires,:domain=>'.scnc-sh.com'}
	         cookies[:UNAME] = {:value=>account.login_name,:expires=>expires,:domain=>'.scnc-sh.com'}
	         cookies[:MLV] = {:value=>account.user.member_lv_id,:expires=>expires,:domain=>'.scnc-sh.com'}
	         cookies[:CUR] = {:value=>account.user.cur,:expires=>expires,:domain=>'.scnc-sh.com'}
	         cookies[:LANG] = {:value=>account.user.lang,:expires=>expires,:domain=>'.scnc-sh.com'}
	       else
	...
	def sign_out
		...
		[:MEMBER,:loginName,:UNAME,:MLV,:CUR,:LANG].each do |e|
			if Rails.env == "production"
				cookies.delete(e.to_s,:domain=>".scnc-sh.com")
			else
    ...
    cookies[:unlogin_url] = {:value=>return_url,:domain=>".scnc-sh.com"}

    #"http://#{subdomain}.#{request.domain}/passport-login.html"
    "http://mall.scnc-sh.com/"
	end
	...
	def site_path
		return "http://mall.scnc-sh.com/" if Rails.env == "development"
		"http://mall.scnc-sh.com/"
	end
